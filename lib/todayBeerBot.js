(function() {
  ...
  todayBeerBot = (function() {
    ...
    todayBeerBot.prototype.getRSS = function(callback) {
      var FeedParser, feedparser, items, req, request, that;
      FeedParser = require('feedparser');
      request = require('request');
      req = request(this.feedList[0].rss);
      that = this;
      ...
      feedparser.on("readable", function() {
        var item, meta, stream, text, _results;
        stream = this;
        meta = this.meta;
        _results = [];
        while (item = stream.read()) {
          text = that._htmlToText(item["atom:content"]["#"]);
          if (that._hasCraftBeerKeyword(text) === true) {
