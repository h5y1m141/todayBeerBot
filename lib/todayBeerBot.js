(function() {
  ...
  todayBeerBot = (function() {
    function todayBeerBot() {
      ...
      this.mongolabkey = conf.mongolabkey;
    ...
    todayBeerBot.prototype.checkIfFeedAlreadyPostOrNot = function(targetFeedURL, callback) {
      ...
      return mongo.connect(uri, {}, function(error, db) {
        return db.collection("shop").find().toArray(function(err, items) {
          var item, _i, _len, _results;
          _results = [];
          for (_i = 0, _len = items.length; _i < _len; _i++) {
            item = items[_i];
            if (item.permalink === targetFeedURL) {
              _results.push(callback(true));
            } else {
              _results.push(void 0);
            }
          }
